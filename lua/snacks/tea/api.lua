...
---@class snacks.tea.api
local M = {}

---@type table<string, snacks.picker.tea.Item>
local cache = setmetatable({}, { __mode = "v" })
local pr_cache = {} ---@type table<string, snacks.picker.tea.Item>

---@type table<string, snacks.tea.api.Config|{}>
local config = {
	base = {
		list = {
			"index",
...
---@param opts snacks.tea.api.Fetch
...
---@param what "pr"
---@param cb fun(items?: snacks.picker.tea.Item[])
...
---@param cb fun(item?: snacks.picker.tea.Item, updated?: boolean)
---@param item snacks.tea.api.View|{number?: number}
...
function M.view(cb, item, opts)
	...
	local it ---@type snacks.tea.PR?

	---@param data? snacks.tea.PR|snacks.tea.PR[]|{}
...
function M.get(item, opts)
	local ret ---@type snacks.picker.tea.Item?
...
---@param cb fun(comments?: snacks.tea.Comment[])
---@param item snacks.tea.api.View
	...
	for _, buf in ipairs(vim.api.nvim_list_bufs()) do
		if vim.api.nvim_buf_is_loaded(buf) then
			if vim.api.nvim_buf_get_name(buf) == item.uri then
				require("snacks.tea.buf").attach(buf, item)
			end
		end
	end
end
...
---@param opts snacks.tea.api.CreatePR
